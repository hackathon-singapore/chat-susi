...
class MessageComposer extends Component {
  ...
  createUserMessage(text, voice) {
    ...
    formatUserMessage({
      ...
      var patt = /translate/i;
      if (patt.test(userMessage.text)) {
        this.props.actions.openModal({ modalType: 'translate' })
      } else {
        this.props.actions
          ...
          .then(
            apis.getSusiReply(userMessage).then(response => {
              formatSusiMessage({
                ...
                console.log(susiMessage, "susiMessage")
...
function mapDispatchToProps(dispatch) {
  return {
    actions: bindActionCreators({...uiActions, ...messageActions}, dispatch),
  };
}
